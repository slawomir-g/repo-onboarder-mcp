...
export class GitCommitCollector {
   ...
   */
  async collect(repoDir: string, limit = 50): Promise<CommitCollectionResult> {
    ...
    const logOptions = [`--format=${format}`, "--numstat", "--no-merges", `-n ${limit}`];
  ...
  private parseLog(rawLog: string, commitDelimiter: string, headerEndDelimiter: string): CommitCollectionResult {
    ...
    for (const chunk of chunks) {
      ...
      if (diffPart) {
        ...
        for (const line of diffLines) {
          ...
          // biome-ignore lint/style/noNonNullAssertion: map entry created immediately above
    ...
    const hotspots = Array.from(fileStatsMap.values()).sort((a, b) => b.commits - a.commits);
